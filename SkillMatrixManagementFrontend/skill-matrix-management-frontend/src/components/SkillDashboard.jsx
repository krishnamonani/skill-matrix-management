...
const SkillDashboard = () => {
    ...
    useEffect(() => {
        const fetchSkills = async () => {
            try {
                const response = await axios.get(`${apiUrl}/skills-by-user/${userId}`);
                const fetchedSkills = response.data.data.map(skill => ({
                    id: skill.id,
                    name: skill.coreSkillName,
                    level: reverseProficiencyMap[skill.selfAssessedProficiency] || 'Beginner'
                }));
    ...
    const handleAddSkill = async () => {
        ...
        try {
            const response = await axios.post(apiUrl, {
                userId,
                coreSkillName: newSkill,
                selfAssessedProficiency: proficiencyMap[newLevel]
            });
            const newSkillData = response.data.data;
            setSkills([...skills, {
                id: newSkillData.id,
                name: newSkillData.coreSkillName,
                level: reverseProficiencyMap[newSkillData.selfAssessedProficiency]
            }]);
            setNewSkill('');
    ...
    const saveEdit = async () => {
        ...
        try {
            await axios.put(`${apiUrl}/${editingId}`, {
                userId,
                coreSkillName: editSkill,
                selfAssessedProficiency: proficiencyMap[editLevel]
            });
